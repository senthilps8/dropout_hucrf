...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    model acrf; model* crf = &acrf; crf->P = P; crf->PP = PP; crf->R = R; crf->RR = RR; crf->A = A; crf->E = E1; crf->E_bias = E1_bias; crf->labE = E2; crf->labE_bias = E2_bias; crf->type = type; crf->K = K; crf->D = D; crf->no_hidden = no_hidden; crf->second_order = true;
    model amean_crf; model* mean_crf = &amean_crf; mean_crf->P = mean_P; mean_crf->PP = mean_PP; mean_crf->R = mean_R; mean_crf->RR = mean_RR; mean_crf->A = mean_A; mean_crf->E = mean_E1; mean_crf->E_bias = mean_E1_bias; mean_crf->labE = mean_E2; mean_crf->labE_bias = mean_E2_bias; mean_crf->type = type; mean_crf->K = K; mean_crf->D = D; mean_crf->no_hidden = no_hidden; mean_crf->second_order = true;
    
    /* Initialize the step sizes */
    double eta_P   = base_eta / (total_length * K);
    double eta_PP  = base_eta / (total_length * K * K);
    //double eta_A   = base_eta / (total_length * K * K * K);
    ...
    for(int iter = 0; iter < max_iter; iter++) {
		...
		/* Print out progress */
		mexPrintf("Herding iteration %d of %d...\n", iter + 1, max_iter);
    ...
    if(nlhs > 1) {
		...
		dims[0] = 1;
		const char* field_names[] = {"pi", "pi2", "tau", "tau2", "A", "E", "labE", "E_bias", "labE_bias", "type"};
		plhs[1] = mxCreateStructArray(1, dims, 10, field_names);		
