function err = ocr_experiment(type, no_hidden, lambda, rho)
%OCR_EXPERIMENT Runs experiment on OCR data set
%
%    err = ocr_experiment(type, no_hidden, lambda, rho)
%
% The function runs an experiment on the OCR data set with the specified 
...
% University of California, San Diego
    ...
    if any(strcmpi(type, {'linear_crf', 'quadr_crf', 'hidden_crf'})) && isempty(lambda)
        ...
        for i=1:length(lambda)
            ...
            switch type
                ...
                case 'quadr_crf'
                    model = train_crf(train_X, train_T, 'quadr_continuous', lambda(i), 250);
    ...
    for fold=1:no_folds
        ...
        switch type
            ...
            case 'quadr_crf'
                model = train_crf(train_X, train_T, 'quadr_continuous', lambda, 500);
                
            case 'hidden_crf'
                model = train_crf(train_X, train_T, 'drbm_continuous', lambda, 500, no_hidden, 'random');
                
            case 'linear_crf_sgd'        
                model = train_crf_sgd(train_X, train_T, 'continuous', lambda, 75, 1e-4, 1);
                
            case 'quadr_crf_sgd'
                model = train_crf_sgd(train_X, train_T, 'quadr_continuous', lambda, 75, 1e-4, 1);
                
            case 'hidden_crf_sgd'
                model = train_crf_sgd(train_X, train_T, 'drbm_continuous', lambda, 75, 1e-3, 1, no_hidden);
        ...
        if any(strcmpi(type, {'linear_crf', 'quadr_crf', 'hidden_crf', 'linear_crf_sgd', 'quadr_crf_sgd', 'hidden_crf_sgd'}))
            ...
            for i=1:length(train_X)
                ...
                if strcmpi(type, 'hidden_perceptron')
                    trn_err = trn_err + sum(viterbi_hidden_crf(train_X{i}, model) ~= train_T{i});
                else
                    trn_err = trn_err + sum(viterbi_crf(train_X{i}, model) ~= train_T{i});
                end
            end
            disp(['Per-character tagging error (training set): ' num2str(trn_err / tot)]);
        end
