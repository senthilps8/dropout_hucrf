 ...
 * Function call:
 *
 *		[pred_test_T, model] = crf_herding_2nd_order(train_X, train_T, test_X, test_T, type, average_models, base_eta, rho, max_iter, burnin_iter);
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	double* PP     = (double*) calloc(K * K,     sizeof(double));
	double* R      = (double*) calloc(K * 1,     sizeof(double));
	double* RR     = (double*) calloc(K * K,     sizeof(double));
	double* A      = (double*) calloc(K * K * K, sizeof(double));
    ...
    mxArray* matlab_PP = mxCreateDoubleMatrix(K, K, mxREAL); double* mean_PP = mxGetPr(matlab_PP);
    mxArray* matlab_R  = mxCreateDoubleMatrix(K, 1, mxREAL); double* mean_R  = mxGetPr(matlab_R);
    mxArray* matlab_RR = mxCreateDoubleMatrix(K, K, mxREAL); double* mean_RR = mxGetPr(matlab_RR);
    dims[0] = K; dims[1] = K; dims[2] = K;
    mxArray* matlab_A = mxCreateNumericArray(3, dims, mxDOUBLE_CLASS, mxREAL); double* mean_A = mxGetPr(matlab_A);
	dims[0] = D; dims[1] = K;
    ...
    model acrf; model* crf = &acrf; crf->P = P; crf->PP = PP; crf->R = R; crf->RR = RR; crf->A = A; crf->E = E; crf->E_bias = E_bias; crf->invS = invS; crf->type = type; crf->K = K; crf->D = D; crf->second_order = true;
    model amean_crf; model* mean_crf = &amean_crf; mean_crf->P = mean_P; mean_crf->PP = mean_PP; mean_crf->R = mean_R; mean_crf->RR = mean_RR; mean_crf->A = mean_A; mean_crf->E = mean_E; mean_crf->E_bias = mean_E_bias; mean_crf->invS = mean_invS; mean_crf->type = type; mean_crf->K = K; mean_crf->D = D; mean_crf->second_order = true;
    ...
    if(mxIsStruct(prhs[4])) {
        ...
        memcpy(PP, (double*) mxGetPr(mxGetField(prhs[4], 0, "pi2")),  K * K *     sizeof(double));
        memcpy(R,  (double*) mxGetPr(mxGetField(prhs[4], 0, "tau")),  K * 1 *     sizeof(double));
        memcpy(RR, (double*) mxGetPr(mxGetField(prhs[4], 0, "tau2")), K * K *     sizeof(double));
        memcpy(A,  (double*) mxGetPr(mxGetField(prhs[4], 0, "A")),    K * K * K * sizeof(double));
    ...
    double eta_PP     = base_eta / (total_length * K * K);    
    double eta_A      = base_eta / (total_length * K * K);
    double eta_E      = base_eta / (total_length * D * K);
    double eta_E_bias = base_eta / (total_length * D * K);
    double eta_invS   = base_eta / (total_length * D * D * K);
    if(is_discrete) { eta_E *= (D / K); eta_E_bias *= (D / K); }
    int pred_interval = min(2000, N / 10);
    
	/* Allocate some working memory */
	double* old_P  = (double*) malloc(K * 1 *     sizeof(double));
	double* old_PP = (double*) malloc(K * K *     sizeof(double));
	double* old_R  = (double*) malloc(K * 1 *     sizeof(double));
	double* old_RR = (double*) malloc(K * K *     sizeof(double));
	double* old_A  = (double*) malloc(K * K * K * sizeof(double));
    ...
    for(int iter = 0; iter < max_iter; iter++) {
		...
		memcpy(old_PP, PP, K * K * sizeof(double));
		memcpy(old_R,   R, K * 1 * sizeof(double));
		memcpy(old_RR, RR, K * K * sizeof(double));
		memcpy(old_A,   A, K * K * K * sizeof(double));
        ...
        for(int i = 0; i < N; i++) {
            ...
            if(is_quadratic) {               
                viterbi_quadr_crf_2nd_order(train_Xi, train_Ti, rho, len, crf, is_discrete, pred_Ti, &L);
            }
            else {
                viterbi_linear_crf_2nd_order(train_Xi, train_Ti, rho, len, crf, is_discrete, pred_Ti, &L);
            }
            for(int j = 0; j < len; j++) {
            ...
            R[(int) train_Ti[len - 1]] += eta_P; 
            if(len > 1) {
                PP[(int) train_Ti[1] * K + (int) train_Ti[0]] += eta_PP;
                RR[(int) train_Ti[len - 1] * K + (int) train_Ti[len - 2]] += eta_PP;                
            }
            for(int j = 2; j < len; j++) {
                A[(int) train_Ti[j] * K * K + (int) train_Ti[j - 1] * K + (int) train_Ti[j - 2]] += eta_A;
            }
			...
			P[pred_Ti[0]] -= eta_P; 
            R[pred_Ti[len - 1]] -= eta_P; 
            if(len > 1) {
                PP[pred_Ti[1] * K + pred_Ti[0]] -= eta_PP;
                RR[pred_Ti[len - 1] * K + pred_Ti[len - 2]] -= eta_PP;          
            }
            for(int j = 2; j < len; j++) {
                A[pred_Ti[j] * K * K + pred_Ti[j - 1] * K + pred_Ti[j - 2]] -= eta_A;
            }
            if(is_discrete) {                                               // for discrete data
                for(int j = 0; j < len; j++) {
			...
			if(!average_models && iter + 1 >= burnin_iter && i % pred_interval == 0) {
				for(int j = 0; j < M; j++) {
                    ...
                    if(is_quadratic)                
                        viterbi_quadr_crf_2nd_order(test_Xi, T, 0.0f, len, crf, is_discrete, pred_Ti, &L);
                    else
                        viterbi_linear_crf_2nd_order(test_Xi, T, 0.0f, len, crf, is_discrete, pred_Ti, &L);
        ...
        for(int i = 0; i < K * K; i++) param_change += abs(PP[i] - old_PP[i]);
        for(int i = 0; i < K * 1; i++) param_change += abs(R[i]  - old_R[i]);
        for(int i = 0; i < K * K; i++) param_change += abs(RR[i] - old_RR[i]);
        for(int i = 0; i < K * K * K; i++) param_change += abs(A[i] - old_A[i]);
		...
		if(iter + 1 >= burnin_iter) {
            if(average_models) {
                ...
                for(int i = 0; i < K * K; i++)     mean_PP[i] = ((ii - 1) / ii) * mean_PP[i] + (1 / ii) * PP[i];
                for(int i = 0; i < K * 1; i++)     mean_R[i]  = ((ii - 1) / ii) * mean_R[i]  + (1 / ii) * R[i];
                for(int i = 0; i < K * K; i++)     mean_RR[i] = ((ii - 1) / ii) * mean_RR[i] + (1 / ii) * RR[i];
                for(int i = 0; i < K * K * K; i++) mean_A[i]  = ((ii - 1) / ii) * mean_A[i]  + (1 / ii) * A[i];
                ...
                for(int i = 0; i < M; i++) {
                    ...
                    double L = 0.0f; double* T = NULL;                    
                    if(is_quadratic)                
                        viterbi_quadr_crf_2nd_order(test_Xi, T, 0.0f, len, mean_crf, is_discrete, tmp_Ti, &L);
                    else
                        viterbi_linear_crf_2nd_order(test_Xi, T, 0.0f, len, mean_crf, is_discrete, tmp_Ti, &L);
	...
	if(nlhs > 1) {
        if(!average_models) {
            ...
            memcpy(mean_PP, PP, K * K * sizeof(double));
            memcpy(mean_R,  R,  K * 1 * sizeof(double));
            memcpy(mean_RR, RR, K * K * sizeof(double));
            memcpy(mean_A,  A,  K * K * K * sizeof(double));
		...
		dims[0] = 1;
        if(is_quadratic) {
            const char* field_names[] = {"invS", "pi", "pi2", "tau", "tau2", "A", "E", "E_bias", "type"};
            plhs[1] = mxCreateStructArray(1, dims, 9, field_names);
            mxSetField(plhs[1], 0, "invS", matlab_invS);
        }
        else {
            const char* field_names[] = {"pi", "pi2", "tau", "tau2", "A", "E", "E_bias", "type"};
            plhs[1] = mxCreateStructArray(1, dims, 8, field_names);		
        }
		mxSetField(plhs[1], 0, "pi",   matlab_P);
		mxSetField(plhs[1], 0, "pi2",  matlab_PP);
		mxSetField(plhs[1], 0, "tau",  matlab_R);
		mxSetField(plhs[1], 0, "tau2", matlab_RR);
