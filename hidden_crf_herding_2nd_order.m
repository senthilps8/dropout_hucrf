function [pred_T, model] = hidden_crf_herding_2nd_order(train_X, train_T, test_X, test_T, type, no_hidden, average_models, base_eta, rho, max_iter, burnin_iter)
%HIDDEN_CRF_HERDING_2ND_ORDER Performs herding in a hidden unit CRF
%
%   [pred_T, model] = hidden_crf_herding_2nd_order(train_X, train_T, test_X, test_T, type, no_hidden, average_models, base_eta, rho, max_iter, burnin_iter)
%   [pred_T, model] = hidden_crf_herding_2nd_order(train_X, train_T,
%   test_X, test_T, model, [], average_models, base_eta, rho, max_iter, burnin_iter)
%
...
% University of California, San Diego
    ...
    for iter=1:max_iter
        ...
        for i=1:n
            ...
            [cur_T, ~, Z_neg] = viterbi_hidden_crf_2nd_order(train_X{i}, model, train_T{i}, rho, EX);
            ...
            if iter >= burnin_iter && ~average_models && ~rem(i, pred_interval)
                for j=1:m
                    sequence = viterbi_hidden_crf_2nd_order(test_X{j}, model);
        ...
        if iter >= burnin_iter
            ...
            if average_models
                ...
                for i=1:m
                    pred_T{i} = viterbi_hidden_crf_2nd_order(test_X{i}, mean_model);
                end
