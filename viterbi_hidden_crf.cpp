 ...
 *
 *      [sequence, L, posterior, Z] = joint_viterbi_hidden_crf(X, model, T, rho, EX)
 *
 */
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    mexPrintf("VITERBI_HIDDEN_CRF: This implementation appears to have a bug!\n");
    ...
    model acrf; model* crf = &acrf; crf->P = P; crf->PP = PP; crf->A = A; crf->E = E; crf->E_bias = E_bias; crf->labE = labE; crf->labE_bias = labE_bias; crf->type = type; crf->K = K; crf->D = D; crf->no_hidden = no_hidden; crf->second_order = false;
    ...
    if(nlhs > 3) {                                          // return hidden unit states
        int dims[] = {no_hidden, len};
        plhs[3] = mxCreateLogicalArray(2, dims);
        mxLogical* ZZ = (mxLogical*) mxGetData(plhs[3]);
        for(int i = 0; i < len; i++) {
